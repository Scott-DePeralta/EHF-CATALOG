...
# ── INJECT INTO HTML ─────────────────────────────────────
def inject(html, old_const, new_js, end_marker):
    s = html.find(f'const {old_const}=[')
    e = html.find(end_marker, s)
    if s == -1 or e == -1:
        print(f'  WARNING: Could not find const {old_const} — skipping')
        return html
...
def main():
    ...
    all_raw = str(flower_rows)+str(preroll_rows)+str(vape_rows)+str(edibles_rows)
    ...
    if data_hash == prev_hash:
        ...
        return
    print(f'Changes detected (hash {prev_hash[:8] or "none"} → {data_hash[:8]}). Rebuilding...')
    ...
    html = inject(html, 'PREROLL',  preroll_js,  '\nconst VAPE=')
    html = inject(html, 'VAPE',     vape_js,     '\nconst EDIBLES=')
    html = inject(html, 'EDIBLES',  edibles_js,  '\nconst EXTRACTS=')
    html = inject(html, 'EXTRACTS', extracts_js, '\nconst SYRUP=')
    html = inject(html, 'SYRUP',    syrup_js,    '\nconst TOPICALS=')
    html = inject(html, 'TOPICALS', topicals_js, '\nconst GELCAPS=')
    html = inject(html, 'GELCAPS',  gelcaps_js,  '\nconst TABS=')
